<?php

namespace app\models;

use yii\base\Model;
use yii\data\ActiveDataProvider;
use app\models\DegreeSubject;

/**
 * DegreeSubjectSearch represents the model behind the search form of `app\models\DegreeSubject`.
 */
class DegreeSubjectSearch extends DegreeSubject
{
    /**
     * {@inheritdoc}
     */
    public function rules()
    {
        return [
            [['degree_subject_id', 'degree_id', 'subject_id', 'department_id'], 'integer'],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = DegreeSubject::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }
        $query->joinWith('degree');
        $query->joinWith('subject');
        $query->joinWith('department');


        // grid filtering conditions
        $query->andFilterWhere([
            'degree_subject_id' => $this->degree_subject_id,
            // 'degree_id' => $this->degree_id,
            // 'subject_id' => $this->subject_id,
            // 'department_id' => $this->department_id,
        ]);
        $query->andFilterWhere(['like', 'degree.name', $this->degree_id]);
        $query->andFilterWhere(['like', 'subject.name', $this->subject_id]);
        $query->andFilterWhere(['like', 'department.name',  $this->department_id]);

        return $dataProvider;
    }
}
